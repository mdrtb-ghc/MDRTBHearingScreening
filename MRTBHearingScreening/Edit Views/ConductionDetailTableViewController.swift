...
class ConductionDetailTableViewController: UITableViewController, ConductionDetailTableViewCellDelegate {
    ...
    var sections = ["Left Ear","Right Ear"]
    ...
    func dbLevelUpdated(cell: UITableViewCell, newDbLevel: Int) {
        if let indexPath = self.tableView.indexPathForCell(cell) {
            rows[indexPath.section][indexPath.row]["value"] = newDbLevel
        }
    }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.section < rows.count {
            let row_section = rows[indexPath.section]
            if indexPath.row < row_section.count {
                
                let row = row_section[indexPath.row]
                cell.Frequency.text = row["label"] as? String ?? "Row \(indexPath.row)"
                
                let dbLevel = row["value"] as? Int ?? UISegmentedControlNoSegment
